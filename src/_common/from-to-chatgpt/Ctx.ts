export default class Ctx {
	...
	conAry!: { from: string, to: string, prompt?: string, transformer?: string }[];
	...
	changeFocusConTransformer(prompt: string, transformer: string) {
		if (!this.focusCon) {
			return;
		}
		const conAry = this.conAry;
		conAry.forEach((con, index) => {
			if (con.from === this.focusCon.fromXpath && con.to === this.focusCon.toXpath) {

				conAry.splice(index, 1, {
					...con,
					prompt:prompt,
					transformer: encodeURIComponent(transformer),
				});
			}
		});
	}
