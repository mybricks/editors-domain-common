	...
	entity.fieldAry
		...
		.forEach(filed => {
			if (filed.bizType === FieldBizType.DATETIME && filed.showFormat) {
				ifString += `;if (key === '${filed.name}') { item[key] = FORMAT_MAP.formatTime(new Date(item[key]), '${filed.showFormat}') }\n`;
			}
		});
	/** mapping 字段列表 */
	mappingFields.forEach(field => {
		...
		entity.fieldAry.forEach(f => {
			if (f.bizType === FieldBizType.DATETIME && f.showFormat) {
				ifString += `;if (key === '${field.name}_${f.name}') { item[key] = FORMAT_MAP.formatTime(new Date(item[key]), '${f.showFormat}') }\n`;
			}
