			...
			if (condition.conditions) {
				...
				if (field) {
					...
					if (entityMapElement.id !== curEntity.id) {
						const mappingField = curEntity.fieldAry.find(f => f.bizType === 'mapping' && f.mapping?.entity?.id === condition.entityId);
			...
			if (sql || mappingFields.length) {
				...
				mappingFields.forEach((mappingField, index) => {
					...
					if (mappingField.mapping?.type === 'primary') {
						const relationField = curEntity.fieldAry.find(f => ['relation', 'SYS_USER'].includes(f.bizType) && f.relationEntityId === mappingField.mapping?.entity?.id);
						...
						/** 与主实体存在关联关系的外键字段 */
						const relationField = entityMap[mappingField.mapping!.entity!.id]?.fieldAry.find(f => f.bizType === 'relation' && f.relationEntityId === curEntity.id);
		...
		if (!whereJoiner) {
			...
			if (sql || mappingFields.length) {
				...
				mappingFields.forEach((mappingField, index) => {
					/** 被关联 */
					if (mappingField.mapping?.type === 'primary') {
						const relationField = curEntity.fieldAry.find(f => f.bizType === 'relation' && f.relationEntityId === mappingField.mapping?.entity?.id);
