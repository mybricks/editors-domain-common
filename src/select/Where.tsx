...
class MyCtx {
  whereEle: HTMLElement

  popEle: HTMLElement

  popFields: { entity: T_Entity }
  ...
  return (
    <div className={css.where} ref={ele => ele && (wCtx.whereEle = ele)}>
      <div className={css.segTitle}>
        ...
        的数据
        <span className={css.addWhere}
              onClick={evt(addWhere).stop}>+</span>
        {
          wCtx.popFields ? (
                ...
                wCtx.popFields.entity.fieldAry.map(field => {
                  ...
                  return (
                    <div key={field.id}
                         ...
                         onClick={!used ? e => addCondition(field) : null}>
                      {field.name}
                    </div>
                  )
                })
...
function Conditions() {
  ...
  const allCons = useComputed(() => {
    return nowValue.conditions.map(condition => {
      ...
      if (oriField) {//存在表切换的情况
        const operators = oriField.getConditionAry()
